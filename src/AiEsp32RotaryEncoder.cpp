// based on https://github.com/marcmerlin/IoTuz code - extracted and modified
// Encoder code
//
//
...
#endif
{
  ...
  if (this->isEnabled) {
    ...
    int8_t ENC_PORT = ((digitalRead(this->encoderBPin)) ? (1 << 1) : 0) |
    ...
    int8_t currentDirection =
        (this->enc_states[(this->old_AB & 0x0f)]);  //-1,0 or 1

    if (currentDirection != 0) {
      // bool ignoreCorrection = false;
      // if (this->encoder0Pos > this->_maxEncoderValue) ignoreCorrection =
      // true; if (this->encoder0Pos < this->_minEncoderValue) ignoreCorrection
      // = true;
      ...
      if (newRotaryPosition != prevRotaryPosition &&
          rotaryAccelerationCoef > 1) {
        ...
        if (currentDirection == lastMovementDirection &&
            currentDirection != 0 && lastMovementDirection != 0) {
          ...
          if (millisAfterLastMotion < accelerationLongCutoffMillis) {
            if (millisAfterLastMotion < accelerationShortCutffMillis) {
              millisAfterLastMotion =
                  accelerationShortCutffMillis;  // limit to maximum
                                                 // acceleration
            }
            if (currentDirection > 0) {
              this->encoder0Pos +=
                  rotaryAccelerationCoef / millisAfterLastMotion;
            } else {
              this->encoder0Pos -=
                  rotaryAccelerationCoef / millisAfterLastMotion;
            }
      ...
      Now it seems better however that -3 divided with 4 will give -1 which is
      not regular -> also readEncoder() is changed to give allowed values It is
      not yet perfect for cycling options but it is much better than before

      optimistic view was that most of the time encoder0Pos values will be near
      to N*encodersteps
      */
      // respect limits
      if ((this->encoder0Pos / this->encoderSteps) >
          (this->_maxEncoderValue / this->encoderSteps)) {
        // Serial.print("circle values limit HIGH");
        // Serial.print(this->encoder0Pos);
        // this->encoder0Pos = this->_circleValues ? this->_minEncoderValue :
        // this->_maxEncoderValue;
        if (_circleValues) {
          // if (!ignoreCorrection){
          int delta =
              this->_maxEncoderValue + this->encoderSteps - this->encoder0Pos;
        ...
        // this->encoder0Pos = this->_circleValues ? (this->_minEncoderValue
        // this->encoder0Pos-this->encoderSteps) : this->_maxEncoderValue;
        //  Serial.print(" -> ");
        //  Serial.println(this->encoder0Pos);
      } else if ((this->encoder0Pos / this->encoderSteps) <
                 (this->_minEncoderValue / this->encoderSteps)) {
        // Serial.print("circle values limit LOW");
        // Serial.print(this->encoder0Pos);
        // this->encoder0Pos = this->_circleValues ? this->_maxEncoderValue :
        // this->_minEncoderValue;
        this->encoder0Pos = this->_circleValues ? this->_maxEncoderValue
                                                : this->_minEncoderValue;
        if (_circleValues) {
          // if (!ignoreCorrection){
          int delta =
              this->_minEncoderValue + this->encoderSteps + this->encoder0Pos;
...
#else
  pinMode(this->encoderAPin,
          (areEncoderPinsPulldownforEsp32 ? INPUT_PULLDOWN : INPUT_PULLUP));
  pinMode(this->encoderBPin,
          (areEncoderPinsPulldownforEsp32 ? INPUT_PULLDOWN : INPUT_PULLUP));
#endif
}

void AiEsp32RotaryEncoder::setBoundaries(long minEncoderValue,
...
void AiEsp32RotaryEncoder::setEncoderValue(long newValue) { reset(newValue); }
...
void AiEsp32RotaryEncoder::setup(void (*ISR_callback)(void),
  ...
  attachInterrupt(digitalPinToInterrupt(this->encoderAPin), ISR_callback,
                  CHANGE);
  attachInterrupt(digitalPinToInterrupt(this->encoderBPin), ISR_callback,
                  CHANGE);
  if (this->encoderButtonPin >= 0)
    // attachInterrupt(digitalPinToInterrupt(this->encoderButtonPin),
    // ISR_button, FALLING);
    attachInterrupt(digitalPinToInterrupt(this->encoderButtonPin), ISR_button,
                    RISING);
}
...
#else
    pinMode(this->encoderButtonPin,
            isButtonPulldown ? INPUT_PULLDOWN : INPUT_PULLUP);
#endif
  }
}

ButtonState AiEsp32RotaryEncoder::currentButtonState() { return buttonState; }
...
void AiEsp32RotaryEncoder::reset(long newValue_) {
  ...
  if (this->encoder0Pos > this->_maxEncoderValue)
    this->encoder0Pos =
        this->_circleValues ? this->_minEncoderValue : this->_maxEncoderValue;
  if (this->encoder0Pos < this->_minEncoderValue)
    this->encoder0Pos =
        this->_circleValues ? this->_maxEncoderValue : this->_minEncoderValue;

  this->lastReadEncoder0Pos = this->readEncoder();
}

void AiEsp32RotaryEncoder::enable() { this->isEnabled = true; }
void AiEsp32RotaryEncoder::disable() { this->isEnabled = false; }
...
bool AiEsp32RotaryEncoder::isEncoderButtonClicked(
  ...
  if (button && buttonReleased) {
    ...
    return true;
  } else if (!button && !buttonReleased &&
             (currentTime - lastClickTime >= maximumWaitMilliseconds)) {
    buttonReleased = true;
  }
